...
Function::Function(std::string name, List params, Expr body, Env* env)
  : _name(std::move(name)), _params(std::move(params)), _body(std::move(body)), _env(env)
{}
...
Expr Function::call(List const& args)
{
  ...
  if (_params.size() != args.size())
    AFCT_ERROR("Expected " << _params.size() << " args to " << _name << " but got " << args.size());
...
NativeFunction::NativeFunction(std::string name, std::function<Expr(List const&)> f)
