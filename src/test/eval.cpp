...
BOOST_AUTO_TEST_CASE(eval_stdlib_structure)
{
  ...
  BOOST_TEST(
      Eval(std::string("(table 1 2 3 4)")) == Eval(std::string("#(1 2 3 4)")));
  BOOST_TEST(Eval(std::string("(length '(1 2))")) == Expr::FromInt(2));
  BOOST_TEST(
      Eval(std::string("(append '(1 2 3 4) 5)")) ==
      Eval(std::string("'(1 2 3 4 5)")));
  BOOST_TEST(
      Eval(std::string("(cons '(1 2) '(3 4))")) == Parse("((1 2) (3 4))"));
...
{
  BOOST_TEST(
      Eval(std::string(R"((cat  "hello " "world"))")) ==
      Parse(R"("hello world")"));
...
{
  BOOST_TEST(
      Eval(std::string("(apply (lambda (x y) (* x y)) '(2 3))")) ==
      Expr::FromInt(6));
  BOOST_TEST(
      Eval(std::string("(map (lambda (x) (* x 2)) '(1 2 3))")) ==
      Parse("(2 4 6)"));
  BOOST_TEST(
      Eval(std::string("(filter (lambda (x) (= x 2)) '(1 2 3))")) ==
      Parse("(2)"));
}
